...
public class AddUserController {
	...
	public void addUser(ActionEvent event) throws Exception {
		...
		if (pswd.length() < 8) {
			passwordNotSame("Error", "Password must be at least 8 characters long");
		} else {
			...
			if (pswd.equals(pswdComfirm)) {
				if ((usrnm.equals("")) || (pswd.equals("")) || (pswdComfirm.equals("")) || (email.equals(""))) {
					...
					if (checkName == false) {
						// CAREFULL!! EMAIL CHECKER DOESNT work 100% if you give letters with accents it
						// will fail!
						...
						// could be better with function that checks if suggested username not already
						// taken as well but not enough time
						passwordNotSame("NOT UNIQUE", "This userName is already Taken! you could try " + usrnm
								+ usrnm.substring(0, 1).toUpperCase());
				}
