...
<html>
	...
	<body>
		<?php
			...
			$query="SELECT * FROM LOGIN_WEBSITE WHERE LoginID ='" .$_GET['a']."'";
			$result = mysqli_query($conn,$query);
			$row = mysqli_fetch_array($result);

			if($userid == $row['LoginID'] & $lengthPassword == strlen($row['Wachtwoord']) & $emailadres == $row['Email']){
				if(ISSET($_POST['submit'])){
					$Userid = $_POST['Userid'];
					$query="SELECT * FROM LOGIN_WEBSITE WHERE LoginID ='" .$Userid."'";
					//echo $query;
					$result = mysqli_query($conn,$query);
					...
					if($_POST['nieuwwachtwoord'] == $_POST['nieuwwachtwoordherhalen']){
						...
						//echo $salt;
						//echo $hash;
						$query = "UPDATE LOGIN_WEBSITE SET Wachtwoord ='" .$hash."', salt ='".$salt."' WHERE LoginID='" .$Userid."'";
						//echo $query;
						$result = mysqli_query($conn,$query);
						...
						echo "<form action='wachtwoord_vergeten_change.php' method='post' enctype='multipart/form-data'>
